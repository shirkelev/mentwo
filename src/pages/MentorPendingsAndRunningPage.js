...
export default function MentorPendingsAndRunningPage ({user}) {
  ...
  const [message, setmessage] = useState(user.pendingMenteesData?.length !== 0 ? '' : "You don't have any pending interviews at the moment");
  return (
    <>
     <div style={{ width: '100vw', height: '100vh' }}>
      <div style={{ padding: '10px'}}>
        <HomeContent headline = {Constants.PENDINGS} list={pendingsList} user={user}/>
        <IconWithMessage message={message} isVisible={user.pendingMenteesData?.length === 0}/>
